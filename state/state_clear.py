            ...
            for filename in os.listdir(task_dir):
                ...
                try:
                    if os.path.isfile(file_path):
                        os.unlink(file_path)
                        owner.log("运行状态", f"成功删除工作任务文件: {file_path}")
                    elif os.path.isdir(file_path):
                        shutil.rmtree(file_path)
                        owner.log("运行状态", f"成功删除工作任务目录: {file_path}")
                except Exception as e:
                    owner.log("运行状态", f"删除 {file_path} 时发生错误: {e}")
        ...
        if os.path.exists(chat_dir):
            for filename in os.listdir(chat_dir):
                ...
                try:
                    if os.path.isfile(file_path):
                        os.unlink(file_path)
                        owner.log("运行状态", f"成功删除聊天记录文件: {file_path}")
                    elif os.path.isdir(file_path):
                        shutil.rmtree(file_path)
                        owner.log("运行状态", f"成功删除聊天记录目录: {file_path}")
                except Exception as e:
                    owner.log("运行状态", f"删除 {file_path} 时发生错误: {e}")
            ...
            for filename in os.listdir(eval_dir):
                ...
                if filename.endswith("_(评估建议).md"):
                    try:
                        if os.path.isfile(file_path):
                            os.unlink(file_path)
                            owner.log("运行状态", f"成功删除提示词的评估建议文件: {file_path}")
                    except Exception as e:
                        owner.log("运行状态", f"删除 {file_path} 时发生错误: {e}")
