...
class ChatMessage:
    ...
    def parse(self, data: str):
        ...
        # Use re.DOTALL to allow . to match newlines, and ^ anchor with re.MULTILINE
        pattern = r'^\[(.*?)\]\:(.*)'
        # re.DOTALL allows (.*) to capture all content including newlines after [role]:
        match = re.match(pattern, data.strip(), flags=re.DOTALL)
        if match:
...
class WorkFile:
    ...
    def __init__(self,
                 file_abstract: dict,  # files_abstract.parquet 中的一行记录
                 ):
        # 命名的一致性
...
class WorkTask:
    ...
    def task_execute_history(self):
        ...
        if file_bytes:
            ...
            if start_index != -1:
                return file_bytes[start_index:]
        else:
            return ""

    # def record_task_execute(self, record: ChatMessage):
    #     """任务执行记录"""
    #     from .global_functions import GlobalFunction
    #     executor_path = GlobalFunction.task_id_to_executor_path(self.task_id)
    ...
    def load_prompt(self, prompt_name: str, owner, state) -> tuple[str, dict]:
        ...
        if input_param:
            ...
            for key in input_param.keys():
                ...
                # 如果状态属性中没有，再从实体属性中获取.
                # owner.xx.yy.zz，首先是直接找owner的owner.owner.xx.yy.zz属性，再找owner.xx.yy.zz的属性
                if value is None:
