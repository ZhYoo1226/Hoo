...
class BaseLLM(ABC):
    def __init__(self, model: str, api_key: str | None = None) -> None:
        ...
        if model not in self.valid_models():
            warning = f"'{model}' is not a valid model for '{self.provider}' provider. Valid models are: {', '.join(self.valid_models())}. openaiSDK.yaml"
            LOG.warning(warning)
            model = self.valid_models()[0]
            LOG.info(f"Using default model '{model}' instead.")
