...
class DomainBuilder:
    ...
    def formalize_predicates(
        ...
        """
        参数	含义
            model	LLM 实例，用于执行文本生成（如 OpenAI 的 GPT）
            domain_desc	领域的自然语言描述（例如“这是积木世界，有积木和手……”）
            prompt_template	一个提示模板，其中包含 {domain_desc}、{types} 等占位符，用于构建最终传给 LLM 的提示
            types	已有的类型定义（可选），用于让 LLM 生成的谓词参数类型与领域类型保持一致
            constants	已有的常量定义（可选）
            predicates	已存在的谓词列表（用于增量添加或避免重复）
            functions	已存在的函数列表（PDDL 中可选的函数）
            syntax_validator	语法验证器对象，用于校验 LLM 输出的正确性
        ...
        for attempt in range(max_retries):
            try:
                ...
                if nl_actions is not None:
                    return nl_actions, llm_output, None
    ...
    def formalize_pddl_action(
        ...
        for attempt in range(max_retries):
            try:
                ...
                spec_results = {}
                spec_results["action"] = action
                spec_results["new_predicates"] = new_predicates
                # run syntax validation if applicable
                validation_info = (True, "All validations passed.")
                if syntax_validator:
                    for error_type in syntax_validator.error_types:
    ...
    """
    该函数是 generate_domain 的辅助方法，主要作用有：

自动化：避免手动编写需求声明，减少错误。

精确性：根据实际使用的语法特征精准添加所需需求，避免遗漏或冗余。

智能合并：能够将多个子需求合并为更高级的 :adl，保持 PDDL 代码简洁。

特别支持：检测是否具备定义动作代价的条件，自动添加 :action-costs 需求。

通过这种方式，用户可以专注于定义领域结构（类型、谓词、动作），而需求部分由程序自动适配，提高了领域生成的可靠性和可维护性
    """
    ...
    #返回一个完整的 PDDL 领域定义字符串，可直接用于规划器或保存为 .pddl 文件
    def generate_domain(
        self,
        domain_name: str,
        types: dict[str, str] | list[dict[str, str]] | None = None,
